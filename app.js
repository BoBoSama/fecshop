...
App({
  ...
  */
	...
	saveHotelInfo: function (request) {
		let hotelInfo = {
			'hotelId': '',
			'hotelName': '',
			'hotelRoom': '',
		...
		let res_hotel_id = request['hotelId']
		let res_hotel_name = request['hotelName']
		let res_hotel_room = request['hotelRoom']

		if (res_hotel_id == 'undefined') res_hotel_id = ''
		if (res_hotel_name == 'undefined') res_hotel_name = ''
		if (res_hotel_room == 'undefined') res_hotel_room = ''

		if (res_hotel_id) {
			hotelInfo['hotelId'] = res_hotel_id
		}
		if (res_hotel_name) {
			hotelInfo['hotelName'] = res_hotel_name
		}
		if (res_hotel_room) {
			hotelInfo['hotelRoom'] = res_hotel_room
		}

		wx.setStorageSync('hotelInfo', hotelInfo);
	},
	getHotelInfo: function () {
		const hotelInfo = wx.getStorageSync('hotelInfo')

		if (!hotelInfo['hotelId']) hotelInfo['hotelId'] = ''
		if (!hotelInfo['hotelName']) hotelInfo['hotelName'] = ''
		if (!hotelInfo['hotelRoom']) hotelInfo['hotelRoom'] = ''
